...
@pytest.mark.asyncio
async def test_self_query_api_end_to_end(api_client):
    from tests.conftest import (
        mock_sentence_transformers,
        mock_qdrant,
        mock_unstructured_partition,
    ...
    # Mock httpx.AsyncClient.post for structured LLM parsing
    mock_response = MagicMock()
    mock_response.status_code = 200
    mock_response.json = lambda: {
        "choices": [{
            "message": {
                "content": '{"query": "Postgres and Qdrant", "filters": {"file_name": "test_hybrid_doc.txt"}}'
            }
        }],
    ...
    try:
        ...
        # 2. Modify strategy to self_query and configure search_type and allowed_keys
        if config_dict.get("retrieval") is None:
            ...
            config_dict["retrieval"]["strategy"] = "self_query"
            config_dict["retrieval"]["config"] = {
                "search_type": "dense",
                "allowed_keys": ["file_name", "page_number"]
            }
        ...
        # 5. Query using self_query
        query_data = {"query": "Find Postgres and Qdrant database engines in test_hybrid_doc.txt"}
        res_query = api_client.post("/api/query", json=query_data)
        assert res_query.status_code == 200
        ...
        print(f"E2E Self-Query Answer: {data['answer']}")
        print(f"E2E Self-Query Sources: {data['sources']}")
    finally:
        # Restore original httpx method
