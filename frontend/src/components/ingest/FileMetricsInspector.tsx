...
interface FileMetricsInspectorProps {
  ...
  setOpenRegistryFiles: (updater: (prev: Record<string, boolean>) => Record<string, boolean>) => void;
}
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden animate-fade-in">
      <div className="px-5 py-4 border-b border-slate-200 dark:border-slate-800 shrink-0 flex justify-between items-center bg-slate-50 dark:bg-slate-950/20">
        ...
        <button
          onClick={() => {
            ...
            setOpenRegistryFiles(prev => ({ ...prev, [selectedFile.id]: false }));
          }}
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-6 scrollbar-thin">
        ...
        <div className="bg-slate-50 dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/60 rounded-xl p-4 shadow-sm space-y-4">
          <div className="text-[10px] font-bold text-primary uppercase tracking-wide">Layout Partition Breakdown</div>
          
          <div className="space-y-3">
            <div className="space-y-1">
              <div className="flex justify-between text-[11px] font-semibold">
                <span className="text-slate-600 dark:text-slate-350">Text elements</span>
                <span className="text-slate-850 dark:text-slate-200 font-bold">{text} ({textPercent}%)</span>
              </div>
              <div className="w-full bg-slate-200 dark:bg-slate-850 h-1.5 rounded-full overflow-hidden">
                <div className="bg-primary h-full transition-all duration-500" style={{ width: `${textPercent}%` }} />
              </div>
            </div>

            <div className="space-y-1">
              <div className="flex justify-between text-[11px] font-semibold">
                <span className="text-slate-600 dark:text-slate-350">Title / Header elements</span>
                <span className="text-slate-850 dark:text-slate-200 font-bold">{title} ({titlePercent}%)</span>
              </div>
              <div className="w-full bg-slate-200 dark:bg-slate-855 h-1.5 rounded-full overflow-hidden">
                <div className="bg-sky-500 h-full transition-all duration-500" style={{ width: `${titlePercent}%` }} />
              </div>
            </div>

            <div className="space-y-1">
              <div className="flex justify-between text-[11px] font-semibold">
                <span className="text-slate-600 dark:text-slate-350">Table elements</span>
                <span className="text-slate-850 dark:text-slate-200 font-bold">{table} ({tablePercent}%)</span>
              </div>
              <div className="w-full bg-slate-200 dark:bg-slate-860 h-1.5 rounded-full overflow-hidden">
                <div className="bg-amber-500 h-full transition-all duration-500" style={{ width: `${tablePercent}%` }} />
              </div>
            </div>

            <div className="space-y-1">
              <div className="flex justify-between text-[11px] font-semibold">
                <span className="text-slate-600 dark:text-slate-350">Image elements</span>
                <span className="text-slate-850 dark:text-slate-200 font-bold">{image} ({imagePercent}%)</span>
              </div>
              <div className="w-full bg-slate-200 dark:bg-slate-865 h-1.5 rounded-full overflow-hidden">
                <div className="bg-purple-500 h-full transition-all duration-500" style={{ width: `${imagePercent}%` }} />
              </div>
            </div>
          </div>
        </div>
